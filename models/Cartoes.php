...
class Cartoes extends model {
    ...
    public function buscaUnicoCondicionado($request){
        $coluna = addslashes( $request['campo'] );
        $condicao = addslashes( $request['condicao'] );
        $valor = addslashes( $request['valor'] );
        $array = array();
        // 
        $sqlA = "SELECT `id` FROM `cartoes` WHERE situacao = 'ativo' AND $coluna = '$condicao' AND posicao = '$valor'";
        // echo $sqlA; exit;
        
        $nomesAux = array();
        $sqlA = self::db()->query($sqlA);
        if($sqlA->rowCount() > 0){  
            // print_r('entrou aqui'); exit;
            $nomesAux = $sqlA->fetch(PDO::FETCH_ASSOC);
